...
class VoucherContainer extends StatelessWidget {
  const VoucherContainer(
      {super.key,
      required this.voucherDesription,
      required this.voucherCode,
      required this.voucherExpire,
      this.discountAmount,
      this.minimumAmount,
      required this.index,
                  ...
                  Column(
                    ...
                    children: [
                        ...
                        child: Consumer<TheatreShowingController>(
                          ...
                          return RichText(
                            ...
                            text: TextSpan(children: [
                              ...
                              data.voucherList[index].selectedOption ==
                                      'Fixed Amount'
                                  ? TextSpan(
                                      text: '$discountAmount₹',
                                      style: const TextStyle(
                      ...
                      Consumer<TheatreShowingController>(
                          builder: (context, data, child) {
                        return data.voucherList[index].selectedOption ==
                                'Fixed Amount'
                            ? Text('Minimum amount to apply $minimumAmount₹',
                                style: const TextStyle(
                                  color: Colors.grey,
                                  fontSize: 13,
                                ))
                            : Text(
                                'Minimum amount to apply $percentageMinimumAmount₹',
                                style: const TextStyle(
                                  color: Colors.grey,
                                  fontSize: 13,
                                ));
            ...
            return SizedBox(
              ...
              child: ElevatedButton(
                  onPressed: () {
                    ...
                    if (value.voucherList[index].selectedOption ==
                        'Fixed Amount') {
                      if (minimumAmount! > value.tottalPrice) {
                        getError(
                            "for this voucher Applying minimum ${value.voucherList[index].minimumAmount} purchase needed",
                            context);
                      } else {
                        Provider.of<TheatreShowingController>(context,
                                listen: false)
                            .voucherApplyingFixedAmount(index);
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            backgroundColor: Colors.green,
                            content: Text(
                                "${value.voucherList[index].couponDescrtion} Applied")));
                      }
                    } else {
                      if (percentageMinimumAmount! > value.tottalPrice) {
                        log('ok');
                        getError(
                            "for this voucher Applying minimum ${value.voucherList[index].percentageMinimumAmount} purchase needed",
                            context);
                      } else {
                        Provider.of<TheatreShowingController>(context,
                                listen: false)
                            .voucherApplyingPercentage(index);
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            backgroundColor: Colors.green,
                            content: Text(
